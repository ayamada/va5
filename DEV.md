...
- usageはinterface.js内にコメントで書いて、それをスクリプトで抽出する？
    ...
    - 方針案
        ...
        - 自前ルールのコメントで書いて自前のスクリプトで抽出する。これでも問題ないが、できれば手間をかけたくない
        - jsdoc式のコメントで書いてから抽出する。まあこれでよいのでは？ただ抽出後の内容が自分にとって扱いやすい形式になっているかはかなり怪しい。もうちょっと調査が必要
        - ...
        - ...
    - その他の方針について
        - 抽出結果はmd書式になっている必要がある
        - このmdの内容はそのまま単体で出力するのではなく、何らかのドキュメントに埋め込む形になる
