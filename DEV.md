...
- ドキュメント自動抽出機能の整備
    - 完了
        - config項目にドキュメントを書く
            - @name getConfigOption とした(後で変更するかも)
        - ファイル名に埋め込めるパラメータのドキュメントを書く
            - @name specialFilename とした(後で変更するかも)
        - 各playオプションのドキュメントを書く
            - @name bgm/se/voice全部共通で playOptions にする方向で(後で変更するかも)。個別にしか存在しない項目は項目の中で「seのみ」とか書く
                - Util内にまとめて書いた
    - TODO
        - 抽出コマンドのMakefile化
            - documentationコマンドをwhich対象として追加
            - とりあえず build/dump.md として出力
                - documentation build -f md --markdown-toc false --sort-order alpha -o build/dump.md src/va5_interface.js src/va5/*.js
                    - まだ
            - ↓と同時に進める必要があるのでは？
        - 抽出したmd断片の整形
            - 不要な部分を除去するルールを決める
                - 最初に自動生成htmlコメントがある → これは先頭2行を削るだけでok
                - `### Parameters` セクションを除去する処理を入れる(子要素も含めて)
                    - まだ
                - セクション見出し( `## bgm` のような)を除去し、その代わりにコンテンツ1行目を見出し化する処理を追加
                    - 何故なら、セクション見出し(自動的に変数名/関数名から取得されるものも、 @name で指定するものも)には、空白文字や記号を入れられない為
                    - まだ
        - 整形後はどうする？README.mdに入れる？どうやって？
            - 悩ましい…
            - 要件としては「利用者が見やすいところから見れる」のであれば、README.mdに入れる必要はない。なので適当なヘッダとフッタをつけて別ファイルとして生成してよいと思う
            - 生成物をbuild配下に置くとリポジトリから見れなくなるので、README.mdと同じ階層に置く方向で。
                - ファイル名は？
                    - USAGE.md みたいな？
                    - REFERENCE.md
                    - ...
                    ...
                    - ...
                    - ...
                - ヘッダとフッタの断片はどこに置く？とりあえず深いところに置いておきたい
                    - src/ 内に入れてしまう？
                - ドキュメント更新し忘れ対策はどうする？
                    - github actionsつかってみる？
                    - 忘れてもよいという事にする？
