...
- 上記以外にも、play系のopts、deviceのstate、bgmのstate、seのstate、これらのkeyの名前をより分かりやすいものに変更したい(少なくとも公開前には何とかしたい)
    - ある程度は変更したが…あとどこが分かりづらい？
        - cancelledとisCancelledとがあり統一が取れていない問題がある。これ系を直す手間をかけるかは微妙なところ
...
- 配布用zipを作るところまでやる
    - Makefileのdepsにzipコマンドを追加
    - 最後にdistをzipに固める
        - ディレクトリ名の変更が必要では…どうする？
...
- mp3
    ...
    - ABR/VBRだと古いedgeでdecodeAudioDataが例外を投げる事がある(正確な条件は不明)。CBR推奨
