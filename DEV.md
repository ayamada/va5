...
va5.config["se-chattering-sec"] のチェックを(可能なら)軽量に行う方法と、
判定に引っかかった時の処理をどうするかを決める必要がある

- チェック方法
    - (とりあえず当初は重いチェックでもよいか？)
    - 全chをなめる？
        - seのchは多くなっても100個程度だから、これでも問題ないか？
    - pathをkeyとして持つ別mapを用意すべきか？うーん…
        - しかしこれしかないのでは？
            - これしかない！
    - ...
    - ...
...
- 処理
    - 単に再生しない → volume/pitch/panが同じならこれでいける。しかしそうでない場合は問題になる…
        - chが共通になる問題がある。これを解決する必要がある
    - 古いSEを消して新しいSEを鳴らす → 負荷が高い。避けたい
    - 古いSEのvolume/pitch/panのみmerge(平均を取る) → SEのパラメータを後から変更できる必要がある。
        - これしかないのでは…
        - chが共通になる問題がある。これを解決する必要がある
            - 共通になってもよいのでは？二回shutdownはできないようにしてあるし、複数箇所から同じchをいじる事があっても、寿命の短いSEなら問題ない(寿命の長いBGMやvoiceだと問題になりそうだが)
        - deviceに「後からvolume/pitch/panを変更する」奴を追加する事
            - 追加した




再生スケジューラの戦略を考える必要がある

- 再生リクエスト→load済か？
    - load済なら即対応して完了
    - 未loadなら…
        - とりあえずstateを生成して登録し、そのchを返す
        - load(path, cont) で、cont内で以下の処理を行う
            - キャンセルリクエストが来ていなければplayする。キャンセルリクエストが来るとstateにcancelledが付く
            - キャンセルリクエストが来ていたらキャンセル処理を行う
        - ...
