...
- ドキュメント自動抽出機能の整備
    - src/va5_document.js にドキュメント登録/取得を用意する方式
        - 登録は各js内に va5.Document.register(...) を直に書く
        - 内容の取得は Makefile から va5.Document.hoge() で
    - 絶対に書かないといけないのは以下。ただしソースとの関係性が薄いなら別記でも問題ない
        - interface.js内にある関数
        - config項目
        - bgm/se/voiceのオプション引数
            - va5.Util.checkUnknownOptsSe() 類のところ
            - これはva5.se()類の子扱いになる必要がある。可能か？
        - その他のusage(これは別にする？)
        - initについて(これは別にする？)
        - ...
        - ...
    ...
    - 出力形式を決める必要がある
        - md？
            - このmdの内容はそのまま単体で出力するのではなく、何らかのドキュメントに埋め込む形になる？(例えばオンラインデモhtml内とか)
                - ベストなのはオンラインデモ内に入れる事だが、今回はどうもオンラインデモがva4ほどきちんとしたものにする体力がなさそう。なので、そこ次第になる？
        - html？
        - ...
        - ...
    - keyのsortルールを決める必要がある
        - アルファベット順…これしかない
    - keyの命名ルールを決める必要がある。特にbgm等の子項目を持てるものについて
        - "va5.se(path, opts)"
        - "va5.se(path, opts) - opts.channel"
            - keyのsortルールが文字列順なので、子要素はこういう感じになる以外にはないと思う。ツリー構造のデータ形式にするのはやめた方がいいと思う
            - ただ、もうちょっと分かりやすくできないものか…
        - ...
        - ...
    - 登録方式はやめて、やはりjsdocsにすべきでは？
        - 登録方式での問題点
            - デプロイ先でヘルプデータを持っていてもほぼ意味がない、その容量を削減したい
        - jsdocs方式での問題点
            - closure-compiler向けのアノテーションを解釈されてしまう
                - とは言え、問題はない筈…
            - va5.Util.checkUnknownOptsSe() 類のところとの連携が難しい
                - 連携は考えなくてよい。エラー時はキー一覧だけ出せばよい、完全なヘルプを出す必要はない
            - なのでこちらで。
    - jsdocsの出力を単一のmdにできるやり方などを確認する事
...
- play系のopts、deviceのstate、bgmのstate、seのstateにて、cancelledとisCancelledとがそれぞれで存在し統一が取れていない等の問題がある
    - 間違った参照はないので動作には問題ない状態。これ系を直す手間をかけるかは微妙なところ
    - 面倒なので直さない方向で。ここに時間をかけるよりも他にもっと時間をかけるべき部分が存在する
...
- mp3
    ...
    - ABR/VBRだと古いedgeでdecodeAudioDataが例外を投げる事がある(正確な条件は不明)。CBR推奨
        ...
        - 最近のedgeはchromiumベースになりこの問題はなくなった。どこまで旧edgeの面倒を見るかによる
